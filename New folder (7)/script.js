...
initeksnimasisatu = teksnimasisatu.innerHTML;
teksnimasisatu.innerHTML = '';
function katateksnimasisatu() {
  new TypeIt('#teksnimasisatu', {
    strings: ['' + initeksnimasisatu],
    startDelay: 50,
    speed: 50,
    cursor: true,
...
function gantifotostikerdua() {
  stikersatu.src = stikerdua.src;
  stikersatu.style = '';
}
...
function tes() {
  if (fungsi == 0) {
    ...
    skrg++;
    if (skrg == 2) {
      setTimeout(katateksnimasisatu, 2300);
    }
    if (skrg != 2 && skrg < 4) {
      setTimeout(smn, 700);
    }
    if (skrg == 4) {
      ...
      setTimeout(gantistikerdua, 2300);
    }
  }
}
function smn() {
...
function popunder() {
  if (!getCookie('popunder')) {
    // periksa apakah popunder sudah ditampilkan sebelumnya
